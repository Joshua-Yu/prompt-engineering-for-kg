...
- `PROMPT_FACTORY_API_HEADERS`: JSON object string with extra headers (e.g., `{"api-key": "..."}`) for custom gateways.
- `PROMPT_FACTORY_ORGANIZATION`: overrides the organization header (falls back to `OPENAI_ORGANIZATION`).
